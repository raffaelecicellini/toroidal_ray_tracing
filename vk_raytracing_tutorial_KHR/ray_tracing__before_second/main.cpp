...
int main(int argc, char** argv)
{
  ...
  // Load points from txt files
  helloVk.loadPoints(CURR_DIRECTORY);   // check what happens with nan and scientific format

  helloVk.createOffscreenRender();
  helloVk.createDescriptorSetLayout();  // change bindings
  helloVk.createGraphicsPipeline(); // change from triangle list to point list?
  helloVk.createUniformBuffer();
  helloVk.createCloudDataBuffer();
  ...
  nvmath::vec4f clearColor = nvmath::vec4f(0.8, 0.8, 0.8, 1.00f);
  ...
  while(!glfwWindowShouldClose(window))
  {
    ...
    // Offscreen render pass
    {
      ...
      helloVk.rasterize(cmdBuf);    // single instance of the scene
